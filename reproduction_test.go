...
func TestInitialRegistration(t *testing.T) {
	...
	require.True(t, os.IsNotExist(err), "File should not be written in MemoryMode")

	// Switch to DiskMode and trigger a write
	...
	require.FileExists(t, env.MainJsPath, "File should be written in DiskMode")
